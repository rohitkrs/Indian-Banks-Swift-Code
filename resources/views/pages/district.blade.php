...
@section('title', 'IFSC Code of '.$BankName.' in '.str_replace("-", " ", $State).', search IFSC Code of '.$BankName.' in '.str_replace("-", " ", $State).' IFSC Code finder')
@section('seo_desc', 'Find here IFSC and MICR code of '.$BankName.' and all bank branches, Search IFSC Code for .$BankName. in india, Find IFSC of All Banks in '.str_replace("-", " ", $State).', search IFSC Code in '.str_replace("-", " ", $State).' List of IFSC Codes by State, District and City')
@section('content')
<div class="row ">
                    ...
                    <label for="branch">City</label>
                        <select class="form-control" id="bank" name="bank" onchange="this.options[this.selectedIndex].value && (window.location = this.options[this.selectedIndex].value);">
                            ...
                            @foreach($districts as $district)
                                <option value="/{!! strtolower(str_replace(' ', '-', $BankName)) !!}/{!! strtolower(str_replace(' ', '-', $State)) !!}/{!! strtolower(str_replace(' ', '-', $district->district)) !!}">{{ Str::upper($district->district) }}</option>
    ...
    <div class="container bg-light d-flex align-items-center p-3 my-3 border border-secondary">
        <div class="col-md-12">
            <div class="top_cities footer_link d-flex col-md-11 m-3">
                <ul>
                <h2><strong>Select District</strong></h2>
                    @if(isset($districts))
                        @foreach($districts as $district)
                            <li><a href="/{!! strtolower(str_replace(' ', '-', $BankName)) !!}/{!! strtolower(str_replace(' ', '-', $State)) !!}/{!! strtolower(str_replace(' ', '-', $district->district)) !!}" title="{{ $district->district }}">{{ $district->district }}</a></li>
                        @endforeach
                    @endif
                </ul>
            </div>
